...
int iic_write( XIicPs *IicPs, u8 Address, u8 Data, s32 ByteCount)
{
	...
	if (Status == XST_SUCCESS) {
    xil_printf("IIC PCA9548 Bus switch Configured and channel changed to HDMI\n\r");
    }
	return XST_SUCCESS;
}
...
int check_hdmi_hpd_status(XIicPs *IicPs, u8 Address)
{
	...
	// Status = iic_read2( IicPs, /*ZC706_HDMI_ADDR*/ 0x39, 0x42, &data, 1);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
...
{
	xil_printf("ADV7511 Basic configuration Started\n\r");
	// Power-up the Tx
	...
	iic_write2(IicPs, Address, 0x18, 0x00); //CSC Enabled (RGB to RGB)
	...
	xil_printf("ADV7511 Basic configuration Done\n\r");

}

/*This function set the parameters specific for the ZC706*/
void configure_adv7511_zc706(XIicPs *IicPs, u8 Address)
{
    // ADV7511 Input / Output Mode

    //RGB 4444 with Separated Syncs
    iic_write2(IicPs, Address, 0x15, 0x00);

    //RGB 444 Output Format, Style 1, 8bpc
//    iic_write2(IicPs, Address, 0x16, 0x30);

//    iic_write2(IicPs, Address, 0x17, 0x02);

    //Video Input Justification: Right justified
//    iic_write2(IicPs, Address, 0x48, 0x08);

	xil_printf("ADV7511 Specific configuration Done\n\r");
...
{
	xil_printf("Waiting for Monitor\n\r");
